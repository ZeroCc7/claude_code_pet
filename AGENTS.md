...
四维属性（剑、丹、体、灵）已接入战斗公式和进化分支判定，但目前固件中**没有任何途径提升**，始终为 0。需要设计成长入口：

- **历练事件选择**：不同事件选项增加对应倾向（如迎战→剑、炼丹→丹、淬体→体、冥想→灵）。
- **击败 boss**：根据战斗轮次或表现奖励属性。
- **AI 任务完成**：根据来源或时长分配倾向。

实现时需同步更新 C++ 固件和 Python 参考模型，保持测试通过。
