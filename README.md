...
- 青云山道：自动前进、四种双选事件、信物提前解锁首领。
- 青云妖狼：全自动战斗，等级、形态和四类成长倾向共同参与结算。
- 首领可以重复轮回挑战，难度递增，奖励每轮重新生成。
- 青云剑极品宝物：通关随机掉落，20 次保底，永久增益。
...
- LittleFS 双槽存档、CRC 校验；V1.1 不迁移 V1.0 存档。
...
灵力和体力均通过被动计时恢复。每 5 分钟恢复 1 点灵力（上限取决于形态）和 5 点体力（上限 100）。满值时不累计恢复时间，关机期间不补算。背包中使用灵草可立即恢复 3 点灵力，回春丹可立即恢复 20 点体力。

### 青云山道历练

按 K3 进入历练页，K1 开始历练。出发消耗 3 点灵力，之后每 3 秒自动前进消耗 1 点灵力，进度增加 2%。灵力归零时自动结束历练，保留当前进度。
...
**受伤修士**：判定值 = 等级 + 体力/10 + 灵倾向/2 + 随机(0–9)，需达到 22。成功获得青云令（可提前解锁首领），未成功获得回春丹。两种结果都增加灵倾向 +2。
...
进度达到 100% 或持有青云令时可挑战青云妖狼。开战需要至少 5 点灵力，每回合消耗 1 点灵力。战斗全自动进行，约 1.2 秒一回合。
...
击败青云妖狼后当前轮数 +1，进度和事件重置，从 0% 开始下一轮。每次通关独立生成奖励，不递减。

**难度缩放**（轮数上限 50）：

- 前 10 轮：妖狼生命每轮 +20%，伤害每轮 +12%。
- 第 11 轮起：生命每轮 +5%，伤害每轮 +3%。

| 轮次 | 生命倍率 | 伤害倍率 | 妖狼 HP（基础 40） |
|---:|---:|---:|---:|
| 1 | 100% | 100% | 40 |
| 5 | 180% | 148% | 72 |
| 10 | 280% | 208% | 112 |
